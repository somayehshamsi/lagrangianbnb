...
#             for e in candidates:
#                 # fractional value f (how "present" edge is in averaged primal history)
#                 if shor_primal_solution is not None and e in shor_primal_solution:
#                     f = shor_primal_solution[e]
#                 else:
#                     f = self.get_fractional_value(e)

#                 # clamp for stability
#                 f = max(0.01, min(0.99, float(f)))

#                 # pseudocosts are "per unit" estimates in BranchAndBound's update
...
#                 # If unseen on a side, give a neutral prior so you don’t get score=0 everywhere
#                 if cu == 0:
#                     pc_up = 1.0
#                 if cd == 0:
#                     pc_dn = 1.0
...
#         self.lagrangian_solver.incumbent_ub = getattr(
#             MSTNode, "global_upper_bound", float("inf")
#         )
...
#         already fixed in, exactly num_nodes - 1 - num_fixed_edges remain.  The
#         previous form went through a component count and arrived at
#         num_nodes - num_fixed_edges, one edge too many, which inflated the
#         estimate by one min_edge_length and pruned nodes that did contain
#         feasible -- sometimes optimal -- trees.
#         """
#         fixed_length = sum(
#             self.lagrangian_solver.edge_attributes[(u, v)][1]
#             for u, v in self.fixed_edges
#         )
#         num_fixed_edges = len(self.fixed_edges)
#         edges_needed = self.num_nodes - 1 - num_fixed_edges
...
#         min_edge_length = float('inf')
#         for u, v, _, l in self.edges:
#             if (u, v) not in self.excluded_edges and (u, v) not in self.fixed_edges:
#                 min_edge_length = min(min_edge_length, l)
...
#             # node does.
#             sim_solver.incumbent_ub = getattr(
#                 MSTNode, "global_upper_bound", float("inf")
#             )
...
#         """Order candidates so that a truncated probe budget is spent on the
#         most informative edges.

#         Strong branching can only afford a handful of probes per node, so which
#         edges it looks at matters as much as how it scores them.  With an
#         indicator available the order is by fractionality; without one it is by
#         the LR fractionality surrogate of get_fractional_value.  The MST
#         candidate set used to be truncated in Kruskal order, i.e. arbitrarily.
#         """
#         if indicator is not None:
...
#         if self.branching_rule in ["strong_branching", "strong_branching_all", "sb_fractional"]:
#             # None unless the sb_fractional branch below builds one; the
#             # MST-candidate variants rank on the LR surrogate instead.
#             normalized_edge_weights = None
...
#             max_sb_candidates = int(getattr(self, "sb_max_candidates", 3))
#             candidate_edges = self._rank_for_strong_branching(
#                 candidate_edges, normalized_edge_weights
#             )[:max_sb_candidates]
#             # Collect edges that lead to pruning
#             edges_to_fix = set()
#             edges_to_exclude = set()
...
#                 if min(cu, cd) >= self.reliability_eta:
#                     reliable_candidates.append(e)
#                 else:
#                     unhistoried.append(e)
