...
#     #             if k >= 2:
#     #                 pos = bisect.bisect_left(lens, lf)
#     #                 if pos <= k - 2:
#     #                     sigma += lf - lens[k - 2]
#     #                 lens.insert(pos, lf)
...
#     #     for e in sorted(TcapA, key=get_len, reverse=True):
#     #         S_seed.append(e)
#     #         sum_seed += get_len(e)
...
#     #             uniq[key] = (set(cset), int(rhs))

#     #     # sum_{S} x <= b implies sum_{S'} x <= b' whenever S' is a subset of S
#     #     # and b <= b', so the strongest candidates are offered first: smallest
#     #     # right-hand side, then largest support.
#     #     final = sorted(uniq.values(), key=lambda t: (t[1], -len(t[0])))
...
#     def generate_cover_cuts(self, mst_edges):
#         """
#         Cover-cut separation as published in the literature.

#         Drop-in replacement for the Section 6 version: same signature, same
#         return contract, so the caller still reduces the cuts under the node
#         fixings, ranks them by violation, caps the active pool and dualizes them
#         exactly as before.  Only the cut itself is weaker.

#         Two strands are combined, both as published.

#         Requejo and Santos [39] generate covers from infeasible Lagrangian trees
#         and dualize them so the subproblem stays a minimum spanning tree under
#         modified edge costs.  The cover is read off the current tree by taking
#         its edges in nonincreasing order of length and accumulating them until
#         the budget is exceeded, giving

#             sum_{e in S} x_e <= |S| - 1,

#         which the generating tree violates by one unit.

#         Agra et al. [5, 6] give the tree-completion cover inequality -- the
#         implicit cover inequality in the matroid form -- together with a static
#         dominance-based extension: admissible edges that are at least as long as
#         the longest cover edge and that close a cycle with the cover take unit
#         coefficients while the right-hand side is unchanged.  The support grows
#         to

#             H = S u { f admissible : l_f >= max_{e in S} l_e,
#                                      f closes a cycle with S },

#         and the inequality becomes sum_{e in H} x_e <= |S| - 1.

#         S satisfies the tree-completion membership test outright, since its own
#         length already exceeds the budget, so no minimum-completion computation
#         is needed to certify it.

#         Deliberately absent, because these are the things being measured
#         against: any notion of minimality for the cover, the deletion search
#         that contracts a cover under the tree-completion certificate, and the
#         two dynamic-threshold lifting rules.  Here the support is only ever
#         enlarged, never contracted, and the acceptance threshold is fixed once
#         from the seed rather than re-evaluated as edges enter.

#         Agra et al.'s second strengthening mechanism, sequential lifting with
#         each coefficient obtained from an auxiliary optimization, is not
#         reproduced: it has no closed form and costs a dual solve per cut.

#         Returns a list holding at most one (support, rhs) pair.
#         """
#         if not mst_edges:
#             return []
...
#         # Residual budget and admissible edge set at the current node.  At the
#         # root, where [5, 6, 39] operate, these are the global budget and the
#         # whole edge set.
#         Bp = self.budget - sum(get_len(e) for e in fixed if e in edge_attr)
...
#         TcapA = [e for e in (norm(x) for x in mst_edges) if e in A]

#         # Nothing to separate unless the current tree breaks the budget.
#         if sum(get_len(e) for e in TcapA) <= Bp + EPS:
#             return []

#         # ------------------------------------------------------------------
#         # Cover from the infeasible Lagrangian tree [39]
#         # ------------------------------------------------------------------
#         S = []
#         total = 0.0
...
#         if not S or total <= Bp + EPS:
#             return []

#         # ------------------------------------------------------------------
#         # Static dominance-based extension [5, 6]
#         #
#         # An edge closes a cycle with the cover exactly when both of its ends
#         # lie in one component of the forest the cover induces, so the test is a
#         # union-find over the cover's own endpoints.
#         # ------------------------------------------------------------------
#         l_max = max(get_len(e) for e in S)

#         parent = {x: x for e in S for x in e}
...
#         for e in A:
#             if e in H or get_len(e) < l_max - EPS:
#                 continue
...
class LagrangianMST:
    ...
    def compute_dantzig_wolfe_solution(self, node):
        ...
        for mst_edges, _is_feasible in self.primal_solutions:
        ...
        # Greedy max-coverage pick of diverse columns.
        max_msts = min(10, len(valid_msts))
        selected_msts = []
        covered = set()
